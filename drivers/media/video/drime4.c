...
static u8 sysfs_isp_cold_boot[20] = {0,};
static u8 sysfs_hotshoe_status[2] = {0,};
static u8 sysfs_cis_serial_number[50] = {0,};
static u8 sysfs_shutter_release_count[10] = {0,};
static u8 sysfs_flash_count[10] = {0,};
...
static int make_SPI_CS_input_output(int val);
	...
	u8 packetID, u16 ID)
{
	...
	int int_factor;
	int noti_lock = 0;
	int ap_int_val, d4_int_val, noti_state_val;
	...
	if (int_factor) {
		...
		d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
		noti_state_val = drime4_spi_get_noti_check_pin();
		cam_err("response timeout. AP INT = %s, Noti(SDA) = %s, D4 INT = %s\n",
			ap_int_val ? "High" : "Low",
	...
	u8 packetID, u16 ID, u32 len, u8 *buf, int log)
{
	...
	err = drime4_write_buf(sd, packetID, ID, len, buf, log);
	CHECK_ERR(err);
...
static int drime4_get_phone_fw_version(struct v4l2_subdev *sd)
{
	...
	if (retVal >= 0) {
		...
		cam_dbg("cannot open firmware file in sdcard. try to open from mmcblk0p17.\n");
...
static int drime4_set_af(struct v4l2_subdev *sd, int val)
{
	...
	cam_info("%s, mode %#x\n", val == 1 ? "start" : "stop", state->focus.mode);
...
static int drime4_set_exif_gps(struct v4l2_subdev *sd, char *buf, u8 size)
{
	...
	for (i; i < size; i++)
		cam_dbg("@@@ buf[%d] = 0x%02x", i, buf[i]);
#endif
	err = drime4_cmdl_res(sd, DRIME4_PACKET_CMD_DATA_SIZE, size + 9);
	CHECK_ERR(err);
...
static int drime4_set_high_temperature(struct v4l2_subdev *sd, int val)
{
	int err = 0;
	cam_dbg("E, value %d\n", val);

	err = drime4_writeb_res(sd, DRIME4_PACKET_DSP_SET,
		DRIME4_SET_HIGH_TEMPERATURE, val);
	CHECK_ERR(err);
...
static int drime4_factory_log_write(struct v4l2_subdev *sd, int val)
{
	...
	int_factor = drime4_wait_ap_interrupt(sd,
		DRIME4_ISP_TIMEOUT);
	if (int_factor) {
		err = drime4_spi_read_burst(&read_val[0], 2);
		if (err < 0)
			cam_err("drime4_spi_read_res length failed\n");
		CHECK_ERR(err);
...
static int drime4_factory_cis_info(struct v4l2_subdev *sd)
{
	struct drime4_state *state = to_state(sd);
	u8 read_val[50] = {0, };
	u8 trans_val[50] = {0, };
	unsigned int qry_value = 0;
	int int_factor;
	int err;
	int ap_int_val, d4_int_val, noti_state_val;
	...
	int_factor = drime4_wait_ap_interrupt(sd,
		DRIME4_ISP_TIMEOUT);
	if (int_factor) {
		err = drime4_spi_read_res(&read_val[0], 0, 0);
		CHECK_ERR(err);

		if (read_val[2] != DRIME4_PACKET_DSP_QUERY_RES) {
		...
		cam_dbg("drime4_write res [%d] packet 0x%02x, ID 0x%02x%02x\n",
			read_val[5], read_val[2], read_val[4], read_val[3]);
	} else {
		cam_err("timeout return\n");
		ap_int_val = gpio_get_value(GPIO_ISP_INT);
		d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
	...
	qry_value = *((int *)(read_val + 6));
	cam_dbg("drime4: qry_value = %d\n", qry_value);
	sprintf(trans_val, "%d", qry_value);
	cam_dbg("drime4: trans_val string = %s\n", trans_val);
	memcpy(sysfs_cis_serial_number, trans_val, sizeof(trans_val));
	cam_dbg("sysfs_cis_serial_number = %s\n", sysfs_cis_serial_number);

	return 0;
}

static int drime4_factory_shutter_release_count(struct v4l2_subdev *sd)
{
	struct drime4_state *state = to_state(sd);
	u8 read_val[50] = {0, };
	u8 trans_val[50] = {0, };
	...
	drime4_cmdw(sd, DRIME4_PACKET_DSP_QUERY,
		DRIME4_QRY_SHUTTER_RELEASE_COUNT);
	...
	sprintf(trans_val, "%d", qry_value);
	cam_dbg("drime4: trans_val string = %s\n", trans_val);
	memcpy(sysfs_shutter_release_count, trans_val, sizeof(trans_val));
	cam_dbg("sysfs_shutter_release_count = %s\n",
		sysfs_shutter_release_count);

	return 0;
}

static int drime4_factory_flash_count(struct v4l2_subdev *sd)
{
	struct drime4_state *state = to_state(sd);
	u8 read_val[50] = {0, };
	u8 trans_val[50] = {0, };
	...
	sprintf(trans_val, "%d", qry_value);
	cam_dbg("drime4: trans_val string = %s\n", trans_val);
	memcpy(sysfs_flash_count, trans_val, sizeof(trans_val));
	cam_dbg("sysfs_flash_count = %s\n", sysfs_flash_count);
...
static int drime4_get_factory_avs_request(struct v4l2_subdev *sd,
	struct v4l2_control *ctrl)
{
	struct drime4_state *state = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
boot_check_retry_factory:
	...
	/* enable ap irq */
	cam_dbg("enable ap irq\n");
	enable_irq(state->isp.ap_irq);
	state->isp.ap_issued = 0;
	...
	cam_dbg("drime4: AVS Value = [%d]\n", avs_value);
	ctrl->value = avs_value;

	if (avs_value >= 0x01 &&  avs_value <= 0x09) {
		err = drime4_writeb_res(sd, DRIME4_PACKET_DSP_SET,
			DRIME4_SET_AVS_VALUE, avs_value);
		CHECK_ERR(err);
	} else {
		if (avs_retry_cnt < 2) {
			cam_err("drime4: avs_retry_cnt = [%d]\n", avs_retry_cnt);
			++avs_retry_cnt;
			goto avs_value_retry_factory;
		}
	}

	cam_dbg("enable d4 irq\n");
	enable_irq(state->isp.d4_irq);
	state->isp.d4_issued = 0;
...
static int drime4_set_burst_boot_param(struct v4l2_subdev *sd, int val)
{
	...
	if (val) {
		...
		err = drime4_cmdl_res(sd, DRIME4_PACKET_CMD_DATA_SIZE,
			param_index + 6);
		CHECK_ERR(err);
		...
		if (pos_x != 0 && pos_y != 0) {
			...
			pos_val |= ((pos_val & 0xFF000000) | buf[3]) << 24;
			pos_val |= ((pos_val & 0xFF0000) | buf[2]) << 16;
			pos_val |= ((pos_val & 0xFF00) | buf[1]) << 8;
			pos_val |= (pos_val & 0xFF) | buf[0];

			err = drime4_writel_res(sd, DRIME4_PACKET_DSP_CMD,
				DRIME4_CMD_UPDATE_AF_POS, pos_val);
			CHECK_ERR(err);
		}
	}
...
static int drime4_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAM_UPDATE_FW:
		...
		else if (ctrl->value == CAM_FACTORY_CS_INFO) {
			err = drime4_factory_cis_info(sd);
			err = drime4_factory_shutter_release_count(sd);
			err = drime4_factory_flash_count(sd);
		} else
			err = 0;
		break;
...
#endif
		if (state->focus.touch == 1 && ctrl->value == 0) {
			err = drime4_set_af(sd, 2);
			state->focus.touch = 0;
		} else
	...
	case V4L2_CID_CAMERA_COLD_BOOT:
		if (ctrl->value)
			state->isp_cold_boot = 1;
		cam_trace("isp_cold_boot : %d", state->isp_cold_boot);
		break;
	...
	case V4L2_CID_CAMERA_HIGH_TEMPERATURE:
		err = drime4_set_high_temperature(sd, ctrl->value);
		break;
...
static int drime4_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_FACTORY_AVS_REQUEST:
		err = drime4_get_factory_avs_request(sd, ctrl);
		break;
...
d4_retry:
	...
	if (int_factor) {
		drime4_spi_set_noti_read_status(1);
#if 1
noti_read_cmd_pin:
		noti_gpio_val = drime4_spi_get_noti_check_pin();
		if (!noti_gpio_val)
			cam_err("Notification gpio read failed.\n");
		cmd_gpio_val = drime4_spi_get_cmd_ctrl_pin();
		cmd_write_val = drime4_spi_get_write_status();
		if (!cmd_gpio_val && !cmd_write_val)
			noti_retry = 1;
		else
			noti_retry = 0;
		...
		if (noti_retry) {
			...
			if (noti_retry_cnt < 20)
				goto noti_read_cmd_pin;
			else
				cam_err("Notification wait timeout: SCL pin is Low");
		} else if (noti_retry_cnt > 0) {
			cam_dbg("Notification wait %d", noti_retry_cnt);
		}
#endif
		...
		drime4_spi_set_noti_read_status(0);
	} else {
#if 1
		ctrl->value = 0;
		return ctrl->value;
...
static int drime4_load_fw(struct v4l2_subdev *sd)
{
	...
	if (int_factor) {
		...
		msleep(500);
		drime4_spi_set_cmd_ctrl_pin(0);
		make_SPI_CS_input_output(0);
		err = drime4_init(sd, state->samsung_app);
		fw_update_result = 1;
	} else {
		ap_int_val = gpio_get_value(GPIO_ISP_INT);
		d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
...
static int drime4_check_lens_fw(struct v4l2_subdev *sd)
{
	...
	if (0 != result) {
		...
		if (0 != result) {
			cam_dbg("Binary file cannot open!\n");
			return 0;
		}
	}
#if 1
...
static int drime4_set_frmsize(struct v4l2_subdev *sd)
{
	struct drime4_state *state = to_state(sd);
	int err;
	int return_liveview = DRIME4_LIVE_MOV_OFF;
	cam_trace("E, %dx%d\n", state->preview->width,
		state->preview->height);

	if (state->isp_mode == DRIME4_LIVEVIEW_MODE)
		return_liveview = DRIME4_LIVEVIEW_MODE;
	else if (state->isp_mode == DRIME4_MOVIE_MODE)
		return_liveview = DRIME4_MOVIE_MODE;
	...
	if (state->sensor_mode == SENSOR_MOVIE) {
		err = drime4_writeb_res(sd, DRIME4_PACKET_DSP_SET,
			DRIME4_SET_MOVIE_SIZE,
			state->movie->reg_val);
		CHECK_ERR(err);
	}

	if (return_liveview == DRIME4_LIVEVIEW_MODE) {
		err = drime4_set_mode(sd, DRIME4_LIVEVIEW_MODE);
		CHECK_ERR(err);
	} else if (return_liveview == DRIME4_MOVIE_MODE) {
	...
	struct v4l2_frmsizeenum *fsize)
{
	...
	*/
	...
	if (state->format_mode == V4L2_PIX_FMT_MODE_PREVIEW) {
		if (state->sensor_mode == SENSOR_MOVIE) {
			if (state->movie == NULL
				/* FIXME || state->movie->index < 0 */)
				return -EINVAL;

			fsize->type = V4L2_FRMSIZE_TYPE_DISCRETE;
			fsize->discrete.width = state->movie->width;
			fsize->discrete.height = state->movie->height;
		} else {
			if (state->preview == NULL
...
boot_check_retry:
		...
		int_factor = drime4_wait_ap_interrupt(sd, 2);
		if (int_factor)
			cam_dbg("AVS: no effect ap interrupt occurred\n");
		int_factor = drime4_wait_d4_interrupt(sd, 2);
		if (int_factor)
			cam_dbg("AVS: no effect d4 interrupt occurred\n");
...
static int drime4_init(struct v4l2_subdev *sd, u32 val)
{
	...
	memset(sysfs_isp_cold_boot, 0, sizeof(sysfs_isp_cold_boot));

#if 1
boot_start_check_retry:
	noti_state_val = drime4_spi_get_noti_check_pin();
	cs_state_val = gpio_get_value(GPIO_CAM_SPI_SSN);
	gpio_free(GPIO_CAM_SPI_SSN);	/* Use SPI CS pin */

	if (cs_state_val && (retry_cnt < 140)) {	/* 5s -> 7s */
		msleep(50);
		retry_cnt++;
	...
	if (cs_state_val) {
		ap_int_val = gpio_get_value(GPIO_ISP_INT);
		d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
		cam_err("ISP Boot start failed. CS = %s, SDA = %s, AP INT = %s, D4 INT = %s\n",
			cs_state_val ? "High" : "Low",
			noti_state_val ? "High" : "Low",
			ap_int_val ? "High" : "Low",
			d4_int_val ? "High" : "Low");
		g_drime4_boot_error = 1;
		sprintf(sysfs_isp_cold_boot, "%s\n", "1");
		pdata->cold_boot(1);
		return -1;
	}
	if (retry_cnt) {
...
#if 0/*def CONFIG_CAM_DEBUG*/
	ap_int_val = gpio_get_value(GPIO_ISP_INT);
	d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
	cs_state_val = gpio_get_value(GPIO_CAM_SPI_SSN);
	cam_dbg("ISP Boot start check. SDA = %s, AP INT = %s, D4 INT = %s, CS = %s\n",
		noti_state_val ? "High" : "Low",
		ap_int_val ? "High" : "Low",
		d4_int_val ? "High" : "Low",
		cs_state_val ? "High" : "Low");
...
boot_check_retry:
	...
	cs_state_val = 0;
#endif
	if (!ap_int_val && (retry_cnt < 50)) {	/* 12s -> 2s */
	...
	if (!ap_int_val) {
		...
		d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
#if 0	/* Use SPI CS pin */
		cam_err("ISP Boot failed. AP INT = %s, Noti(SDA) = %s, D4 INT = %s, CS = %s\n",
			ap_int_val ? "High" : "Low",
			noti_state_val ? "High" : "Low",
			d4_int_val ? "High" : "Low",
			cs_state_val ? "High" : "Low");
#else
		cam_err("ISP Boot failed. AP INT = %s, Noti(SDA) = %s, D4 INT = %s\n",
			ap_int_val ? "High" : "Low",
			noti_state_val ? "High" : "Low",
			d4_int_val ? "High" : "Low");
#endif
		...
		noti_state_val = drime4_spi_get_noti_check_pin();
		d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
		cam_dbg("ISP Boot success. AP INT = %s, Noti(SDA) = %s, D4 INT = %s, CS = %s\n",
			ap_int_val ? "High" : "Low",
			noti_state_val ? "High" : "Low",
			d4_int_val ? "High" : "Low",
			cs_state_val ? "High" : "Low");
...
#if 0	/* Use SPI CS pin */
	cs_state_val = gpio_get_value(GPIO_CAM_SPI_SSN);
	cam_dbg("ISP Boot done check. CS = %s\n",
		cs_state_val ? "High" : "Low");
#endif
...
static ssize_t drime4_camera_rear_isp_cold_boot(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", sysfs_isp_cold_boot);
}

static ssize_t drime4_camera_cis_info(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", sysfs_cis_serial_number);
}

static ssize_t drime4_camera_capture_info(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", sysfs_shutter_release_count);
}

static ssize_t drime4_camera_light_info(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", sysfs_flash_count);
}
...
static DEVICE_ATTR(auto_recovering, S_IRUGO,
	drime4_camera_rear_isp_cold_boot, NULL);
static DEVICE_ATTR(cis_info, S_IRUGO, drime4_camera_cis_info, NULL);
static DEVICE_ATTR(capture_info, S_IRUGO,
	drime4_camera_capture_info, NULL);
static DEVICE_ATTR(light_info, S_IRUGO,
	drime4_camera_light_info, NULL);
/*
 * drime4_probe
...
#if 0	/* Use SPI CS pin */
	gpio_free(GPIO_CAM_SPI_SSN);
	err = gpio_request(GPIO_CAM_SPI_SSN, "SSN");
	if (err) {
		printk(KERN_DEBUG "%s  failed to request gpio(GPIO_CAM_SPI_SSN)\n",
			__func__);
	}
...
static int __devexit drime4_remove(struct i2c_client *client)
{
...
#if 0	/* Use SPI CS pin */
	make_SPI_CS_input_output(1);
#endif
	if (state->isp_cold_boot)
		pdata->cold_boot(1);
	else
		pdata->power_on_off(0);
	...
	ap_int_val = gpio_get_value(GPIO_ISP_INT);
	d4_int_val = gpio_get_value(GPIO_RESERVED_1_INT);
	cam_dbg("ISP power off. SDA = %s, AP INT = %s, D4 INT = %s\n",
		noti_state_val ? "High" : "Low",
		ap_int_val ? "High" : "Low",
		d4_int_val ? "High" : "Low");
#endif

#if 1	/* SPI/I2C pin output low at power off */
	if (system_rev > 7) {
		gpio_direction_output(GPIO_CAM_SCL, 0);
		gpio_direction_output(GPIO_CAM_SDA, 0);
		gpio_free(GPIO_CAM_SCL);
		gpio_free(GPIO_CAM_SDA);
	} else {
		gpio_direction_output(GPIO_GSENSE_SCL_18V, 0);
		gpio_direction_output(GPIO_GSENSE_SDA_18V, 0);
		gpio_free(GPIO_GSENSE_SCL_18V);
		gpio_free(GPIO_GSENSE_SDA_18V);
	}
#else
...
static int __init drime4_mod_init(void)
{
	...
	if (!drime4_dev) {
		...
		if (device_create_file(drime4_dev, &dev_attr_cis_info) < 0) {
			cam_warn("failed to create device file, %s\n",
					dev_attr_cis_info.attr.name);
		}

		if (device_create_file(drime4_dev, &dev_attr_capture_info) < 0) {
			cam_warn("failed to create device file, %s\n",
					dev_attr_capture_info.attr.name);
		}

		if (device_create_file(drime4_dev, &dev_attr_light_info) < 0) {
			cam_warn("failed to create device file, %s\n",
					dev_attr_light_info.attr.name);
		}

		if (device_create_file(drime4_dev, &dev_attr_auto_recovering) < 0) {
			cam_warn("failed to create device file, %s\n",
					dev_attr_auto_recovering.attr.name);
		}
	}
#ifdef CONFIG_SWITCH_EXT_MIC
