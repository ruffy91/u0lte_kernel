...
int drime4_spi_set_noti_read_status(int val)
{
	g_spi_noti_reading = val;
	return g_spi_noti_reading;
}
	...
	if (retry == 1) {
		...
		if (retry_cnt < 5) {
			...
			if (retry_cnt == 5 || retry_cnt == 1000) {
				...
				pr_err("drime4 retry : noti reading = %d, boot_error = %d\n",
					g_spi_noti_reading, g_drime4_boot_error);
			}
		...
		if (retry_cnt < 1000) {
			...
			pr_err("drime4 retry_fail : ap_int = %d, d4_int = %d, noti = %d, scl = %d\n",
				ap_gpio_val, d4_gpio_val,
				noti_gpio_val, g_cmd_ctrl_pin);
			pr_err("drime4 retry_fail : noti reading = %d, boot_error = %d\n",
				g_spi_noti_reading, g_drime4_boot_error);
...
gpio_check_burst_retry:
	...
	if (retry == 1) {
		...
		if (retry_cnt < 5) {
			...
			if (retry_cnt == 5 || retry_cnt == 1000) {
				pr_err("drime4 retry : ap_int = %d, d4_int = %d, noti = %d, scl = %d, noti reading = %d\n",
					ap_gpio_val, d4_gpio_val,
					noti_gpio_val, g_cmd_ctrl_pin,
					g_spi_noti_reading);
			}
		...
		if (retry_cnt < 1000)
			...
			pr_err("drime4 retry_fail : ap_int = %d, d4_int = %d, noti = %d, scl = %d, noti reading = %d\n",
				ap_gpio_val, d4_gpio_val,
				noti_gpio_val, g_cmd_ctrl_pin,
				g_spi_noti_reading);
			pr_err("drime4_spi Write busy fail: packetID 0x%02x, payload len %d\n",
...
int drime4_spi_read_length(int size)
{
	...
	if (size <= 0 || size > 4) {
		pr_err("drime4_spi Read Length : error size of length - %d\n",
			size);
		goto exit_err;
	}
