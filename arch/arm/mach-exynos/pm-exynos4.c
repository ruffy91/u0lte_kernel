...
#if defined(CONFIG_MACH_KONA) || defined(CONFIG_MACH_GD2)
static void exynos4_pm_check_wakeup_pending(void)
{
	int n;
	...
	for (n = 0; n < 10; ) {
		if ((__raw_readl(S5P_EINT_PEND(0)) == 0) &&
			...
			printk(KERN_DEBUG "%s[%d] : Success - Clear %d\n",
					__func__, __LINE__, ++n);
			break;
		} else {
			exynos4_pm_rewrite_eint_pending();
			printk(KERN_DEBUG "%s[%d] : Try to clear %d\n",
					__func__, __LINE__, ++n);
			if (n == 10) {
				printk(KERN_ERR "Couldn't wait.\n");
				BUG();
			}
		}
	}
...
#if defined(CONFIG_MACH_M0_CTC)
{
	...
	if ((__raw_readl(S5P_WAKEUP_STAT) == 0x0)
			&& (soc_is_exynos4212() || soc_is_exynos4412()))
		exynos4_pm_check_wakeup_pending();
