...
static int drime4_spi_dev_init(int enable)
{
	...
	if (ret) {
		printk(KERN_ERR "failed to request gpio(GPIO_CAM_SPI_MISO)\n");
		return ret;
	}
	...
	if (enable) {
...
#else
		s3c_gpio_cfgpin(GPIO_CAM_SPI_SSN, S3C_GPIO_SFN(2));
#endif
#endif
		s3c_gpio_cfgpin(EXYNOS4_GPB(7), S3C_GPIO_SFN(2));
		s3c_gpio_setpull(EXYNOS4_GPB(7), S3C_GPIO_PULL_UP);
		...
		CAM_CHECK_ERR_RET(ret, "output GPIO_CAM_SPI_SSN");
		ret = gpio_direction_output(EXYNOS4_GPB(7), 0);
		CAM_CHECK_ERR_RET(ret, "output GPIO_CAM_SPI_MOSI");
		ret = gpio_direction_output(EXYNOS4_GPB(6), 0);
		CAM_CHECK_ERR_RET(ret, "output GPIO_CAM_SPI_MISO");
#else
...
static int drime4_cold_power(int enable)
{
	struct regulator *regulator;
	int ret = 0;
	int gpio_val = 0;
	...
	ret = gpio_request(D4_COLD, "D4COLD");
	if (ret) {
		printk(KERN_ERR "failed to request gpio(D4_COLD)\n");
		return ret;
	}
	...
	if (enable) {
		/* gpio init */
		ret = gpio_direction_output(GPIO_CAM_PCU_MRB, 1);
		CAM_CHECK_ERR_RET(ret, "output GPIO_CAM_PCU_MRB High");
		ret = gpio_direction_output(D4_COLD, 1);
		CAM_CHECK_ERR_RET(ret, "output D4_COLD High");
		ret = gpio_direction_output(GPIO_CAM_POWER_ON, 0);
		CAM_CHECK_ERR_RET(ret, "output GPIO_CAM_POWER_ON Low");

		/* cold boot start */
		ret = gpio_direction_output(GPIO_CAM_PCU_MRB, 0);
		CAM_CHECK_ERR_RET(ret, "output GPIO_CAM_PCU_MRB Low");
		...
		ret = gpio_direction_output(D4_COLD, 0);
		CAM_CHECK_ERR_RET(ret, "output D4_COLD Low");
		...
		ret = gpio_direction_output(GPIO_CAM_POWER_ON, 1);
		CAM_CHECK_ERR_RET(ret, "output GPIO_CAM_POWER_ON High");
